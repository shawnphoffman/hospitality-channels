...
export async function POST(request: Request) {
	...
	const { profileId, programId } = body as { profileId: string; programId?: string }
	...
	if (programId) {
		...
		const [latestRender] = await db
			.select()
			.from(schema.jobs)
			.where(and(eq(schema.jobs.programId, programId), eq(schema.jobs.type, 'render-program'), eq(schema.jobs.status, 'completed')))
		...
		const job = {
			...
			payload: {
				...
				durationSec: (latestRender.payload as Record<string, unknown>)?.durationSec ?? 30,
	...
	// Clip publish (existing behavior)
