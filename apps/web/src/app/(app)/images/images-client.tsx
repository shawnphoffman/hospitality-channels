		...
		<>
			...
			{initialAssets.length === 0 ? (
				<div className="rounded-xl border border-dashed border-slate-700 p-12 text-center">
					<p className="text-slate-400">No images yet. Place images in the assets folder and click &quot;Scan Assets Folder&quot;, or upload files directly.</p>
				</div>
			) : (
					...
					{initialAssets.map(asset => (
						<div key={asset.id} className="group relative rounded-xl border border-slate-800 bg-slate-900 p-3">
							<div className="mb-2 overflow-hidden rounded-lg bg-slate-800">
								...
								<img src={`/api/assets/serve?path=${encodeURIComponent(asset.originalPath)}`} alt="" className="aspect-video w-full object-cover" loading="lazy" />
							</div>
							<p className="truncate text-sm text-white">{asset.originalPath.split('/').pop()}</p>
							<p className="text-xs text-slate-500">
								{asset.type}
								{dimensions[asset.id] && (
									<span className="ml-1">&middot; {dimensions[asset.id].w}&times;{dimensions[asset.id].h}</span>
								)}
							</p>
