		...
		<>
			...
			{initialAssets.length === 0 ? (
				<div className="rounded-xl border border-dashed border-slate-700 p-12 text-center">
					<p className="text-slate-400">No audio files yet. Place audio in the assets folder and click &quot;Scan Assets Folder&quot;, or upload files directly.</p>
				</div>
			) : (
					...
					{initialAssets.map(asset => (
						<div key={asset.id} className="group relative rounded-xl border border-slate-800 bg-slate-900 p-3">
							<div className="mb-2 flex items-center gap-3 rounded-lg bg-slate-800 p-3">
								...
								<audio src={`/api/assets/serve?path=${encodeURIComponent(asset.originalPath)}`} controls className="h-8 flex-1" preload="none" />
							</div>
